...
const MAIN_MENU = Markup.keyboard([[BUTTON_ADD, BUTTON_VIEW], [BUTTON_DELETE]]).resize();
...
type UserMode = "idle" | "await_photo" | "await_description" | "await_delete_number";
...
function pickLargestPhoto(photos: Array<{ file_id: string; file_unique_id: string }>) {
  return photos[photos.length - 1];
}

function getOrCreateUserState(chatId: number, userId: number): UserSession {
...
function resetUserState(chatId: number, userId: number): void {
...
function addPendingPhoto(state: UserSession, photo: PendingPhoto): boolean {
...
function buildGalleryReplyMarkup(userId: number, page: number, totalPages: number, itemsOnPage: number) {
  ...
  const numberButtons = Array.from({ length: itemsOnPage }, (_, index) => {
    ...
    return Markup.button.callback(String(number), `gallery_pick:${userId}:${page}:${index}`);
  });
  ...
  if (totalPages > 1) {
    const navButtons = [];
    if (page > 0) {
      navButtons.push(Markup.button.callback("⬅️", `gallery_page:${userId}:${page - 1}`));
    }
...
async function renderEmptyGalleryMessage(ctx: any, messageId: number | null): Promise<void> {
  if (!messageId) {
...
async function finalizeAlbumBuffer(bufferKey: string): Promise<void> {
  ...
  if (messageId) {
    try {
      await bot.telegram.editMessageMedia(
        ...
        messageId,
        undefined,
        { type: "photo", media: selectedRow.file_id, caption },
...
async function startDeleteFlow(ctx: any): Promise<void> {
...
bot.command("inline", async (ctx: any) => {
  const total = await getPhotosCount(ctx.from.id);
  ...
  await ctx.reply(
    "Натисни кнопку нижче, щоб вставити фото через inline-режим:",
    Markup.inlineKeyboard([
      [Markup.button.switchToCurrentChat("Вставити в цей чат", "")],
      [Markup.button.switchToChat("Вибрати інший чат", "")]
    ])
  );
});

bot.command("diag", async (ctx: any) => {
  const total = await getPhotosCount(ctx.from.id);
  const me = bot.botInfo || (await bot.telegram.getMe());
  const inlineEnabled = Boolean((me as any).supports_inline_queries);
...
function parseWebhookUrl(rawUrl: string): { domain: string; path: string } {
